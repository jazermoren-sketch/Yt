...
async function answer(interaction, qid, index) {
  ...
  if (correct) seasonEngine.addScore(interaction.user.id, earned, earned);
...
async function startMultiplayer(interaction, id) {
  ...
  game.started = true;
  game.questions = questionEngine.getRandomQuestions(game.totalQuestions, {
...
async function finishMultiplayer(interaction, game) {
  ...
  for (const p of ranking) {
    ...
    u.xp += p.score;
    if (p.correct > 0) {
      u.hintTokens = (u.hintTokens || 0) + p.correct;
    }
    storage.saveUser(u);
